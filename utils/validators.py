...
class FileSizeValidator:
    ...
    def __call__(self, value):
        if (self.min_size_MB) and (value.size < self.min_size_MB *1024*1024):
            raise ValidationError(f"파일 '{value.name}'의 용량이 최소 용량 {self.min_size_MB}MB를 미만해요.")

        if (self.max_size_MB) and (value.size > self.max_size_MB *1024*1024):
            raise ValidationError(f"파일 '{value.name}'의 용량이 최대 용량 {self.max_size_MB}MB를 초과해요.")
